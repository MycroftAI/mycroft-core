...
from mycroft.tts.mimic_tts import (
    Mimic,
    download_subscriber_voices,
    get_mimic_binary,
...
class TestMimic(unittest.TestCase):
    ...
    def test_subscriber(self, mock_thread, _, mock_device_api):
        ...
        mock_thread.assert_called_with(
            target=download_subscriber_voices, args=["trinity"]
        )
        ...
        with mock.patch("mycroft.tts.mimic_tts.exists") as mock_exists:
            ...
            self.assertEqual(
                m.args, [trinity_mimic, "-voice", "trinity", "-psdur", "-ssml"]
            )
