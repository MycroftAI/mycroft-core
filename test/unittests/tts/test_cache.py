...
class TestCache(TestCase):
    ...
    def _load_persistent_cache(self, tts_config):
        ...
        tts_cache = TextToSpeechCache(
            tts_config=tts_config, tts_name="Test", audio_file_type="wav"
        )
    ...
    def test_curate_cache(self, curate_mock):
        ...
        files = {
            "kermit": tts_cache.define_audio_file("kermit"),
        ...
        # was removed
        curate_mock.return_value = [files["kermit"].path, files["gobo"].path]
        tts_cache.curate()
        curate_mock.assert_called_with(
            tts_cache.temporary_cache_dir, min_free_percent=100
        )

        # Verify that the "hashes" kermit and gobo was removed from the
        # dict of hashes.
        self.assertEqual(
            tts_cache.cached_sentences, {"fozzie": (files["fozzie"], None)}
        )
...
class TestCacheContains(TestCase):
    ...
    def setUp(self):
        ...
        for sentence_hash, (audio_file, phoneme_file) in files.items():
            self.tts_cache.cached_sentences[sentence_hash] = (audio_file, phoneme_file)
