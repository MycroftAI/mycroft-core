...
class TestNiceNumberFormat(unittest.TestCase):
    ...
    def test_no_speech(self):
        ...
        self.assertEqual(
            ...
            "6",
            "should format 6.0 as 6 not {}".format(nice_number(6.0, speech=False)),
        )
    ...
    def test_unknown_language(self):
        ...
        """
        self.assertEqual(
            nice_number(5.5, lang="as-fd"),
            "5.5",
            "should format 5.5 as 5.5 not {}".format(nice_number(5.5, lang="as-df")),
        )
...
class TestPronounceNumber(unittest.TestCase):
    ...
    def test_convert_decimals(self):
        ...
        self.assertEqual(
            pronounce_number(21.234, places=3), "twenty one point two three four"
        )
        self.assertEqual(
            pronounce_number(21.234, places=4), "twenty one point two three four"
        )
        self.assertEqual(
            pronounce_number(21.234, places=5), "twenty one point two three four"
        )
        ...
        self.assertEqual(
            pronounce_number(-21.234, places=1), "minus twenty one point two"
        )
        self.assertEqual(pronounce_number(-21.234, places=0), "minus twenty one")
        self.assertEqual(
            pronounce_number(-21.234, places=3), "minus twenty one point two three four"
        )
        self.assertEqual(
            pronounce_number(-21.234, places=4), "minus twenty one point two three four"
        )
        self.assertEqual(
            pronounce_number(-21.234, places=5), "minus twenty one point two three four"
        )
    ...
    def test_convert_hundreds(self):
        ...
        self.assertEqual(
            pronounce_number(209996),
            "two hundred and nine " "thousand, nine hundred " "and ninety six",
        )
        self.assertEqual(
    ...
    def test_convert_scientific_notation(self):
        ...
        self.assertEqual(
            pronounce_number(33, scientific=True),
    ...
    def test_large_numbers(self):
        ...
        self.assertEqual(
            pronounce_number(1000000000000, short_scale=True), "one trillion"
        )
        # TODO maybe beautify this
        self.assertEqual(
            pronounce_number(1000001, short_scale=True), "one million, one"
        )
...
class TestNiceDateFormat(unittest.TestCase):
    ...
    def test_convert_times(self):
        ...
        self.assertEqual(
            nice_time(dt, speech=False, use_24hour=True, use_ampm=True), "13:22"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=True), "thirteen twenty two"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=False), "thirteen twenty two"
        )
        ...
        self.assertEqual(
            nice_time(dt, speech=False, use_24hour=True, use_ampm=True), "13:00"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=True), "thirteen hundred"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=False), "thirteen hundred"
        )
        ...
        self.assertEqual(
            nice_time(dt, speech=False, use_24hour=True, use_ampm=True), "13:02"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=True), "thirteen zero two"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=False), "thirteen zero two"
        )
        ...
        self.assertEqual(
            nice_time(dt, speech=False, use_24hour=True, use_ampm=True), "00:02"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=True), "zero zero zero two"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=False), "zero zero zero two"
        )
        ...
        self.assertEqual(
            nice_time(dt, speech=False, use_24hour=True, use_ampm=True), "01:02"
        )
        self.assertEqual(
            nice_time(dt, use_24hour=True, use_ampm=True), "zero one zero two"
        )
        self.assertEqual(
    ...
    def test_nice_date(self):
        for lang in self.test_config:
            ...
            i = 1
            while self.test_config[lang].get("test_nice_date") and self.test_config[
                lang
            ]["test_nice_date"].get(str(i)):
                ...
                now = (
                    None
                    if not np
                    else datetime.datetime(np[0], np[1], np[2], np[3], np[4], np[5])
                )
                print(
                    "Testing for "
                    + lang
        ...
        for lang in self.test_config:
            set_default_lf_lang(lang)
            for dt in (
                datetime.datetime(2017, 12, 30, 0, 2, 3) + datetime.timedelta(n)
                for n in range(368)
            ):
    ...
    def test_nice_date_time(self):
        for lang in self.test_config:
            set_default_lf_lang(lang)
            i = 1
            while self.test_config[lang].get(
                "test_nice_date_time"
            ) and self.test_config[lang]["test_nice_date_time"].get(str(i)):
    ...
    def test_nice_year(self):
        for lang in self.test_config:
            set_default_lf_lang(lang)
            i = 1
            while self.test_config[lang].get("test_nice_year") and self.test_config[
                lang
            ]["test_nice_year"].get(str(i)):
...
class TestNiceDurationFuncs(unittest.TestCase):
    def test_nice_duration(self):
        ...
        self.assertEqual(
            nice_duration(1, resolution=TimeResolution.MINUTES), "under a minute"
        )
        ...
        self.assertEqual(
            nice_duration(5000), "one hour twenty three minutes and twenty seconds"
        )
        self.assertEqual(nice_duration(5000, speech=False), "1:23:20")
        self.assertEqual(
            nice_duration(50000),
            "thirteen hours fifty three minutes and twenty seconds",
        )  # nopep8
        self.assertEqual(
            nice_duration(50000, resolution=TimeResolution.MINUTES),
            "thirteen hours fifty three minutes",
        )
        self.assertEqual(
            nice_duration(50000, resolution=TimeResolution.HOURS), "thirteen hours"
        )
        self.assertEqual(nice_duration(50000, speech=False), "13:53:20")
        self.assertEqual(
            nice_duration(500000),
            "five days eighteen hours fifty three minutes and twenty seconds",
        )  # nopep8
        ...
        self.assertEqual(
            nice_duration(360000.254, resolution=TimeResolution.SECONDS, speech=False),
            "4d 4h",
        )
        self.assertEqual(
            nice_duration(
                360000.254325, resolution=TimeResolution.MILLISECONDS, speech=False
            ),
            "4d 4:00:00.254",
        )
        self.assertEqual(
            nice_duration(
                360365.254, resolution=TimeResolution.MILLISECONDS, speech=False
            ),
            "4d 4:06:05.254",
        )
        ...
        self.assertEqual(
            nice_duration(0, resolution=TimeResolution.MINUTES), "zero minutes"
        )
        self.assertEqual(
            nice_duration(30, resolution=TimeResolution.MINUTES), "under a minute"
        )

        # test clock output
        self.assertEqual(
            nice_duration(
                60, resolution=TimeResolution.HOURS, clock=True, speech=False
            ),
            "0:01:00",
        )
        self.assertEqual(
            nice_duration(
                1, resolution=TimeResolution.MINUTES, clock=True, speech=False
            ),
            "0:01",
        )
        self.assertEqual(
            nice_duration(
                0.25, resolution=TimeResolution.HOURS, clock=True, speech=False
            ),
            "0:00:00",
        )
        self.assertEqual(
            nice_duration(
                0.25, resolution=TimeResolution.MINUTES, clock=True, speech=False
            ),
            "0:00",
        )
        ...
        self.assertEqual(
            nice_duration(
                0.25, resolution=TimeResolution.MILLISECONDS, clock=True, speech=False
            ),
            "0:00.250",
        )
        self.assertEqual(
            nice_duration(
                60, resolution=TimeResolution.YEARS, clock=True, speech=False
            ),
            "0y",
        )
    ...
    def test_nice_duration_dt(self):
        ...
        self.assertEqual(
            nice_duration_dt(
                datetime.datetime(2019, 12, 25, 20, 30),
        ...
        self.assertEqual(
            nice_duration_dt(
                datetime.datetime(2019, 1, 1), date2=datetime.datetime(2018, 1, 1)
            ),
            "one year",
        )
        self.assertEqual(
        ...
        self.assertEqual(
            nice_duration_dt(
                datetime.datetime(2019, 1, 2), date2=datetime.datetime(2018, 1, 1)
            ),
            "one year one day",
        )

        self.assertEqual(
            nice_duration_dt(datetime.datetime(1, 1, 1), datetime.datetime(1, 1, 1)),
            "zero seconds",
        )
        self.assertEqual(
            nice_duration_dt(
                datetime.datetime(1, 1, 1), datetime.datetime(1, 1, 1), speech=False
            ),
            "0:00",
        )
...
class TestErrorHandling(unittest.TestCase):
    @unittest.skip(
        "Put back when Lingua Franca deprecates " "'lang=None' and 'lang=Invalid'"
    )
