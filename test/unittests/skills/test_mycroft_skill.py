...
class TestMycroftSkill(unittest.TestCase):
    ...
    def test_load_regex_from_file_multiple(self):
        self.check_regex_from_file(
            "valid/multiple.rx", ["(?P<AMultipleTest1>.*)", "(?P<AMultipleTest2>.*)"]
        )
    ...
    def test_load_regex_full(self):
        self.check_regex(
            join(self.regex_path, "valid"),
            ["(?P<AMultipleTest1>.*)", "(?P<AMultipleTest2>.*)", "(?P<ASingleTest>.*)"],
        )
    ...
    def test_load_vocab_file_single(self):
        self.check_read_vocab_file(
            join(vocab_base_path(), "valid/single.voc"), [["test"]]
        )

    def test_load_vocab_from_file_single_alias(self):
        self.check_read_vocab_file(
            join(vocab_base_path(), "valid/singlealias.voc"), [["water", "watering"]]
        )

    def test_load_vocab_from_file_multiple_alias(self):
        self.check_read_vocab_file(
            join(vocab_base_path(), "valid/multiplealias.voc"),
    ...
    def test_load_vocab_full(self):
        self.check_vocab(
            join(self.vocab_path, "valid"),
            {
                "Asingle": [["test"]],
                "Asinglealias": [["water", "watering"]],
                "Amultiple": [["animal"], ["animals"]],
                "Amultiplealias": [["chair", "chairs"], ["table", "tables"]],
            },
        )
    ...
    def _test_intent_file(self, s):
        ...
        expected_results = [
            {
                "file_name": join(
                    dirname(__file__), "intent_file", "vocab", "en-us", "test.intent"
                ),
    ...
    def test_voc_match(self):
        ...
        self.assertTrue(
            s.voc_match("would you please turn off the lights", "turn_off_test")
        )
    ...
    def test_voc_match_exact(self):
        ...
        self.assertFalse(
            s.voc_match(
                "would you please turn off the lights", "turn_off_test", exact=True
            )
        )
