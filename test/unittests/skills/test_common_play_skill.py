...
class TestCommonPlay(TestCase):
    ...
    def test_handle_start_playback(self):
        ...
        start_playback(
            Message("play:start", data={"phrase": phrase, "skill_id": "asdf"})
        )
        ...
        start_playback(
            Message(
                "play:start", data={"phrase": phrase, "skill_id": self.skill.skill_id}
            )
        )
...
class TestCPSQuery(TestCase):
    ...
    def test_play_query_match(self):
        ...
        self.skill.CPS_match_query_phrase.return_value = (
            "until doomsday",
            CPSMatchLevel.TITLE,
        )
        self.query_phrase(Message("play:query", data={"phrase": phrase}))
