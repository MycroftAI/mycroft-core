...
def check_converse_request(message, skill_id):
    return (
        message.msg_type == "skill.converse.request"
        and message.data["skill_id"] == skill_id
    )
...
class ConversationTest(TestCase):
    ...
    def test_converse(self):
        ...
        def response(message, return_msg_type):
            ...
            atari = Message(
                return_msg_type, {"skill_id": "atari_skill", "result": True}
            )
        ...
        utterance_msg = Message(
            "recognizer_loop:utterance", data={"lang": "en-US", "utterances": hello}
        )
    ...
    def test_converse_error(self):
        ...
        def response(message, return_msg_type):
            ...
            amiga = Message(
                return_msg_type,
                {"skill_id": "amiga_skill", "error": "skill id does not exist"},
            )
            atari = Message(
                return_msg_type, {"skill_id": "atari_skill", "result": False}
            )
    ...
    def test_reset_converse(self):
        ...
        def response(message, return_msg_type):
            c64 = Message(
                return_msg_type,
                {"skill_id": "c64_skill", "error": "skill id does not exist"},
            )
            atari = Message(
                return_msg_type, {"skill_id": "atari_skill", "result": False}
            )
        ...
        reset_msg = Message(
            "mycroft.speech.recognition.unknown", data={"lang": "en-US"}
        )
        self.intent_service.bus.wait_for_response.side_effect = response
