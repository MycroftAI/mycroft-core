...
class TestSettingsDownloader(MycroftUnitTestBase):
    ...
    def _check_api_called(self):
        self.assertListEqual(
            [call.get_skill_settings()], self.downloader.api.method_calls
        )
    ...
    def _check_message_bus_events(self, remote_skill_settings):
        self.assertListEqual(
            ["mycroft.skills.settings.changed"], self.message_bus_mock.message_types
        )
        self.assertListEqual(
            [remote_skill_settings], self.message_bus_mock.message_data
        )
