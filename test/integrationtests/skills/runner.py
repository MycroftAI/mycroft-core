...
desc = (
    "Standalone test utility for Mycroft Skills.  This will execute the "
...
def load_test_environment(skill):
    ...
    if exists(test_env_path):
        ...
        if (
            hasattr(module, "test_runner")
            and callable(module.test_runner)
            or hasattr(module, "test_setup")
            and callable(module.test_setup)
        ):
            test_env = module
    return test_env
...
def discover_tests():
    ...
    for skill in skills:
        print("Searching for tests under: " + os.path.join(skill, "test/intent/*.json"))
        test_intent_files = [
            f for f in sorted(glob.glob(os.path.join(skill, "test/intent/*.json")))
        ]
    ...
    def __new__(mcs, name, bases, d):
        ...
        for skill in tests.keys():
            ...
            for example in tests[skill]:
                ...
                test_name = "test_Intent[%s:%s]" % (skill_name, test_filename)
...
class IntentTestSequence(unittest.TestCase, metaclass=IntentTestSequenceMeta):
    ...
    def setUpClass(cls):
        ...
        for s in cls.loader.skills:
            if s.root_dir in cls.test_envs and hasattr(
                cls.test_envs[s.root_dir], "test_setup"
            ):
                try:
