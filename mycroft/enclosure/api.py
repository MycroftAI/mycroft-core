...
class EnclosureAPI:
    ...
    def system_reset(self):
        ...
        self.bus.emit(
            Message("enclosure.system.reset", context={"destination": ["enclosure"]})
        )

    def system_mute(self):
        """Mute (turn off) the system speaker."""
        self.bus.emit(
            Message("enclosure.system.mute", context={"destination": ["enclosure"]})
        )

    def system_unmute(self):
        """Unmute (turn on) the system speaker."""
        self.bus.emit(
            Message("enclosure.system.unmute", context={"destination": ["enclosure"]})
        )
    ...
    def eyes_on(self):
        ...
        self.bus.emit(
            Message("enclosure.eyes.on", context={"destination": ["enclosure"]})
        )

    def eyes_off(self):
        """Turn off or hide the eyes."""
        self.bus.emit(
            Message("enclosure.eyes.off", context={"destination": ["enclosure"]})
        )
    ...
    def eyes_narrow(self):
        ...
        self.bus.emit(
            Message("enclosure.eyes.narrow", context={"destination": ["enclosure"]})
        )
    ...
    def eyes_fill(self, percentage):
        ...
        if percentage < 0 or percentage > 100:
            raise ValueError(
                "percentage ({}) must be between 0-100".format(str(percentage))
            )
        self.bus.emit(
            Message(
                "enclosure.eyes.fill",
    ...
    def eyes_reset(self):
        ...
        self.bus.emit(
            Message("enclosure.eyes.reset", context={"destination": ["enclosure"]})
        )

    def eyes_spin(self):
        """Make the eyes 'roll'"""
        self.bus.emit(
            Message("enclosure.eyes.spin", context={"destination": ["enclosure"]})
        )
    ...
    def eyes_volume(self, volume):
        ...
        if volume < 0 or volume > 11:
            raise ValueError("volume ({}) must be between 0-11".format(str(volume)))
        self.bus.emit(
            Message(
    ...
    def mouth_reset(self):
        ...
        self.bus.emit(
            Message("enclosure.mouth.reset", context={"destination": ["enclosure"]})
        )
        self.display_manager.set_active(self.name)
    ...
    def mouth_talk(self):
        ...
        self.bus.emit(
            Message("enclosure.mouth.talk", context={"destination": ["enclosure"]})
        )
        self.display_manager.set_active(self.name)
    ...
    def mouth_think(self):
        ...
        self.bus.emit(
            Message("enclosure.mouth.think", context={"destination": ["enclosure"]})
        )
        self.display_manager.set_active(self.name)
    ...
    def mouth_listen(self):
        """Show a 'thinking' image or animation"""
        self.bus.emit(
            Message("enclosure.mouth.listen", context={"destination": ["enclosure"]})
        )
        self.display_manager.set_active(self.name)
    ...
    def mouth_smile(self):
        ...
        self.bus.emit(
            Message("enclosure.mouth.smile", context={"destination": ["enclosure"]})
        )
        self.display_manager.set_active(self.name)
    ...
    def mouth_display_png(
        self, image_absolute_path, invert=False, x=0, y=0, refresh=True
    ):
    ...
    def get_eyes_color(self):
        ...
        """
        message = Message(
            "enclosure.eyes.rgb.get",
            context={"source": "enclosure_api", "destination": "enclosure"},
        )
