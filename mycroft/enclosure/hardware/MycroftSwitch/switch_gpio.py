...
class Switch:
    ...
    def __init__(self, debounce=100):
        ...
        self.board_type = config["enclosure"].get("board_type", "sj201r4")
        # GPIO pin numbers
        if self.board_type == "sj201r4":
            self._SW_VOL_DOWN = 22
            self._SW_VOL_UP = 23
            self._SW_ACTION = 24
            self._SW_MUTE = 25
            self._XMOS_POWER = 16  # Enable1V
            self._XMOS_RESET = 27  # Reset XMOS
        else:
            self._SW_ACTION = 22
            self._SW_VOL_UP = 23
            self._SW_VOL_DOWN = 24
            self._SW_MUTE = 25
            self._XMOS_POWER = 13
            self._XMOS_RESET = None  # Not used
