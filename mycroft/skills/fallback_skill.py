...
class FallbackSkill(MycroftSkill):
    ...
    def make_intent_failure_handler(cls, bus):
        ...
        def handler(message):
            ...
            bus.emit(
                message.forward(
                    "mycroft.skill.handler.start", data={"handler": "fallback"}
                )
            )
            ...
            with stopwatch:
                sorted_handlers = sorted(
                    cls.fallback_handlers.items(), key=operator.itemgetter(0)
                )
                ...
                for handler in handlers:
                    ...
                    bus.emit(
                        message.forward(
                            "mycroft.skill.handler.complete",
                            data={"handler": "fallback", "exception": warning},
                        )
                    )
            ...
            if message.context.get("ident"):
                ...
                report_timing(
                    ident, "fallback_handler", stopwatch, {"handler": handler_name}
                )
