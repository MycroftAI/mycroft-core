...
class IntentServiceInterface:
    ...
    def register_adapt_keyword(self, vocab_type, entity, aliases=None):
        ...
        """
        aliases = aliases or []
        self.bus.emit(Message("register_vocab", {"start": entity, "end": vocab_type}))
...
class IntentQueryApi:
    ...
    def get_adapt_intent(self, utterance, lang="en-us"):
        ...
        msg = Message(
            ...
            context={"destination": "intent_service", "source": "intent_api"},
        )

        resp = self.bus.wait_for_response(
            msg, "intent.service.adapt.reply", timeout=self.timeout
        )
    ...
    def get_padatious_intent(self, utterance, lang="en-us"):
        ...
        resp = self.bus.wait_for_response(
            msg, "intent.service.padatious.reply", timeout=self.timeout
        )
        data = resp.data if resp is not None else {}
        if not data:
            LOG.error("Intent Service timed out!")
    ...
    def get_intent(self, utterance, lang="en-us"):
        ...
        resp = self.bus.wait_for_response(
            msg, "intent.service.intent.reply", timeout=self.timeout
        )
        data = resp.data if resp is not None else {}
        if not data:
            LOG.error("Intent Service timed out!")
    ...
    def get_skills_manifest(self):
        ...
        resp = self.bus.wait_for_response(
            msg, "intent.service.skills.reply", timeout=self.timeout
        )
        data = resp.data if resp is not None else {}
        if not data:
            LOG.error("Intent Service timed out!")
    ...
    def get_active_skills(self, include_timestamps=False):
        ...
        resp = self.bus.wait_for_response(
            msg, "intent.service.active_skills.reply", timeout=self.timeout
        )
        data = resp.data if resp is not None else {}
        if not data:
            LOG.error("Intent Service timed out!")
    ...
    def get_adapt_manifest(self):
        ...
        resp = self.bus.wait_for_response(
            msg, "intent.service.adapt.manifest", timeout=self.timeout
        )
        data = resp.data if resp is not None else {}
        if not data:
            LOG.error("Intent Service timed out!")
    ...
    def get_padatious_manifest(self):
        ...
        resp = self.bus.wait_for_response(
            msg, "intent.service.padatious.manifest", timeout=self.timeout
        )
        data = resp.data if resp is not None else {}
        if not data:
            LOG.error("Intent Service timed out!")
