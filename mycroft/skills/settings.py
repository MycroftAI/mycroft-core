...
class SettingsMetaUploader:
    ...
    def __init__(self, skill_directory: str, skill_name: str):
        ...
        self.sync_enabled = self.config["server"].get("sync_skill_settings", False)
        if not self.sync_enabled:
            LOG.info(
                "Skill settings sync is disabled, settingsmeta will " "not be uploaded"
            )
    ...
    def msm_skill_display_name(self):
        ...
        if self._msm_skill_display_name is None:
            skills = {skill.path: skill for skill in self.msm.local_skills.values()}
...
class SkillSettingsDownloader:
    ...
    def __init__(self, bus):
        ...
        if not self.sync_enabled:
            LOG.info(
                "Skill settings sync is disabled, backend settings will "
                "not be downloaded"
            )
    ...
    def _emit_settings_change_events(self, remote_settings):
        ...
        for skill_gid, skill_settings in remote_settings.items():
            ...
            if settings_changed:
                ...
                message = Message(
                    "mycroft.skills.settings.changed", data={skill_gid: skill_settings}
                )
