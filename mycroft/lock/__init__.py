...
# Time-stamp: <2017-04-05 15:14:59 dmendyke>   -*- mode: python; -*-
...
##
# Required Modules
from signal import getsignal, signal, SIGKILL, SIGINT, SIGTERM  # system signals
...
class Signal( object ) :  # python 3+ class Signal
  ...
  def __init__( self, sig_value, func ) :
    ...
    func:  The user supplied function that will act as the new signal handler.
    """
  ...
  def __call__( self, signame, sf ) :
    ...
    except KeyboardInterrupt as kbi : pass
...
class Lock( object ) :  # python 3+ 'class Lock'
  ...
  def exists( self ) :
    """
    Check to see if the PID lock file currently exists.  If it does than
    send a SIGTERM signal to the process defined by the value in the lock file.
    Catch the keyboard interrupt exception to prevent propagation if stopped
    by use of Ctrl-C.
    """
    if not os.path.isfile( self.path ) : return
    with open( self.path, 'r' ) as L :
      try : os.kill( int( L.read() ), SIGKILL )
  ...
  def touch( self ) :
    """
    If needed, create the '/tmp/mycroft' directory than open the lock file
    for writting and store the current process ID (PID) as text.
    """
  ...
  def create( self ) :
    """
    Checks to see if a lock file for this service already exists, if so have it
    killed.  In either case write the process ID of the current service process
    to to the existing or newly created lock file in /tmp/mycroft/
    """
  ...
  def delete( self, *args ) :
    ...
    *args: Ignored.  Required as this fuction is called as a signel handler.
    """
    with open( self.path, 'r' ) as L :
