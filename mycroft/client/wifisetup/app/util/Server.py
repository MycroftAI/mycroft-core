...
class WSHandler(tornado.websocket.WebSocketHandler):
    ...
    def message_switch(self,message):
        ...
        if self.is_match("'ap_on'", message) is True:
            ...
            WiFi = wpaClientTools()
            network_id = WiFi.wpa_cli_add_network(self.client_iface)['stdout'].strip()
            LOGGER.info( network_id)
            LOGGER.info( WiFi.wpa_cli_set_network(self.client_iface, str(network_id), 'ssid', ssid))
            LOGGER.info( WiFi.wpa_cli_set_network(self.client_iface, str(network_id), 'psk', passphrase))
            LOGGER.info( WiFi.wpa_cli_enable_network(self.client_iface, network_id))
            x = 15
            while x > 0:
                try:
                    if WiFi.wpa_cli_status(self.client_iface)['wpa_state'] == 'COMPLETED':
