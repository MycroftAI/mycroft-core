...
def init_screen():
    ...
    if curses.has_colors():
        ...
        for i in range(1, min(curses.COLOR_PAIRS, curses.COLORS)):
            curses.init_pair(i, i, bg)

        # To get the correct color pair, just use the curses color constant:
        #
        # +---------------+----------------------------+
        # |   Constant    |           Color            |
        # +---------------+----------------------------+
        ...
        # +---------------+----------------------------+
        #
        # All pairs have black background.
        # Pair 0 (COLOR_BLACK) does not exist, it's pointless (black on black),
        # but also windows-curses does not support pair 0.

        CLR_HEADING = curses.color_pair(curses.COLOR_WHITE)
        CLR_CHAT_RESP = curses.color_pair(curses.COLOR_YELLOW)
        CLR_CHAT_QUERY = curses.color_pair(curses.COLOR_CYAN)
        CLR_FIND = curses.color_pair(curses.COLOR_YELLOW)
        CLR_CMDLINE = curses.color_pair(curses.COLOR_CYAN)
        CLR_INPUT = curses.color_pair(curses.COLOR_CYAN)
        CLR_LOG1 = curses.color_pair(curses.COLOR_GREEN)
        CLR_LOG2 = curses.color_pair(curses.COLOR_MAGENTA)
        CLR_LOG_DEBUG = curses.color_pair(curses.COLOR_YELLOW)
        CLR_LOG_ERROR = curses.color_pair(curses.COLOR_RED)
        CLR_LOG_CMDMESSAGE = curses.color_pair(curses.COLOR_RED)
        CLR_METER_CUR = curses.color_pair(curses.COLOR_RED)
        CLR_METER = curses.color_pair(curses.COLOR_YELLOW)
    ...
    try:
        while True:
            ...
            if code == 27:    # Hitting ESC twice clears the entry line
                ...
                scr.erase()
            elif code == 24 or code == 3:  # Ctrl+X or Ctrl+C (Exit)
                if find_str:
