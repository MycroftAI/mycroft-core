    ...
    def _synthesize(self, text):
        ...
        with requests.post(_API_URL, headers=headers, data=data, stream=True) as resp:
            if resp.status_code != 200:
                raise Exception("Request to Yandex TTS failed: code: %d, body: %s" % (resp.status_code, resp.text))
    ...
    def validate_connection(self):
        ...
        if api_key is not None:
            ...
            if r.status_code == 400: # Authorized, but bad request
                ...
                raise Exception("Unexpected HTTP code from Yandex TTS (%d)" % r.status_code)
        else:
