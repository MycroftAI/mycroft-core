...
res_path = os.path.abspath(
    os.path.join(os.path.abspath(__file__), "..", "..", "res", "text", "en-us")
)
...
def download_audio(cache_audio_dir, cache_text_file):
    ...
    if os.path.isfile(cache_text_file) and os.path.exists(cache_audio_dir):
        if not os.listdir(cache_audio_dir):
            ...
            with open(cache_text_file, "r") as fp:
                ...
                for each_dialog in all_dialogs:
                    ...
                    key = str(
                        hashlib.md5(each_dialog.encode("utf-8", "ignore")).hexdigest()
                    )
        ...
        missing_path = (
            cache_text_file if not os.path.isfile(cache_text_file) else cache_audio_dir
        )
