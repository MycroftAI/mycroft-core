...
if hash pactl && [[ ${RUN_AS_ROOT} -eq 1 ]] ; then
    ...
    mlog " - $(echo "Pulse Audio Sinks:" && pactl list sinks | grep -e ^Sink -e 'device.description' -e 'product_name' -e udev.id -e State:)"
    mlog " - $(echo "Pulse Audio Sources:" && pactl list sources| grep -e ^Sourc -e 'device.description' -e 'product_name' -e udev.id -e State:)"
else 
