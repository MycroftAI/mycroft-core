...
class LogFileReader:
    ...
    def check_for_inclusion(self, earliest_ts, script_args):
        ...
        matches_inclusion_string = script_args.include is not None and any(
            [i in self.log_msg for i in script_args.include]
        )
        matches_exclusion_string = script_args.exclude is not None and any(
            [e in self.log_msg for e in script_args.exclude]
        )
        msg_parts = self.log_msg.split(" | ")
        process = None if len(msg_parts) < 3 else msg_parts[2].strip()
        include_process = script_args.process is None or (
            process is not None and process == script_args.process
        )

        return (
            emitted_after_start_ts
            and (script_args.include is None or matches_inclusion_string)
            and (script_args.exclude is None or not matches_exclusion_string)
            and include_process
        )
...
class LogWriter:
    ...
    def _check_inclusion_criteria(self, log_msg):
        msg_parts = log_msg.split(" | ")
        include = self.script_args.include is None or any(
            [i in log_msg for i in self.script_args.include]
        )
        if self.script_args.last_boot:
            ...
            if first_boot_message or last_boot_message:
                include = True
        exclude = self.script_args.exclude is not None and any(
            [e in log_msg for e in self.script_args.exclude]
        )
        process = None if len(msg_parts) < 3 else msg_parts[2].strip()
        process_match = self.script_args.process is None or (
...
def _define_script_args():
    ...
    arg_parser.add_argument(
        "--process", help="Show only the logs for the specified process", type=str
    )
    arg_parser.add_argument(
        "--last-boot",
        action="store_true",
